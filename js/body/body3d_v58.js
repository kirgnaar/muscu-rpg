/* ══════════════════════════════════════════════════════════════════════════
   MUSCU RPG — body/body3d_v58.js
   ENGINE v5.5 — Muscle Split Edition (ES5 Stable)
   ══════════════════════════════════════════════════════════════════════════ */
...
var BODY3D = {
  ...
  mapping: {
    ...
    'glutes': 'Fessiers', 'calves_l': 'Mollets', 'calves_r': 'Mollets',
  ...
  buildAthleticBody: function() {
    ...
    // BRAS - SPLIT BICEPS / TRICEPS (v58)
    // Rayon 0.09 | Position 45° OUT | Offset Z pour séparation
    // Biceps (Front)
    addPart(cylGeo(0.05, 0.045, 0.36), -0.44, 1.6, 0.03, 0, -0.785, 'biceps_l', true);
    addPart(cylGeo(0.05, 0.045, 0.36), 0.44, 1.6, 0.03, 0, 0.785, 'biceps_r', true);
    // Triceps (Back)
    addPart(cylGeo(0.05, 0.045, 0.36), -0.44, 1.6, -0.03, 0, -0.785, 'triceps_l', true);
    addPart(cylGeo(0.05, 0.045, 0.36), 0.44, 1.6, -0.03, 0, 0.785, 'triceps_r', true);
    ...
    // JAMBES - SPLIT QUADS / ISCHIOS (v58)
    // Quads (Front)
    addPart(cylGeo(0.12, 0.10, 0.7), -0.18, 0.85, 0.05, 0, 0.05, 'quads_l', true);
    addPart(cylGeo(0.12, 0.10, 0.7), 0.18, 0.85, 0.05, 0, -0.05, 'quads_r', true);
    // Ischios (Back)
    addPart(cylGeo(0.12, 0.10, 0.7), -0.18, 0.85, -0.05, 0, 0.05, 'hams_l', true);
    addPart(cylGeo(0.12, 0.10, 0.7), 0.18, 0.85, -0.05, 0, -0.05, 'hams_r', true);

    // MOLLETS
    addPart(cylGeo(0.1, 0.08, 0.4), -0.2, 0.05, 0, 0, 0.02, 'calves_l', true);
    addPart(cylGeo(0.1, 0.08, 0.4), 0.2, 0.05, 0, 0, -0.02, 'calves_r', true);
